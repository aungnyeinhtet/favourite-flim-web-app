  ...
  return (
    <div className="w-full">
      ...
      {item.tagline && item.tagline.length > 0 && (
        <h3 className="text-PRIMARY-TEXT font-semibold lg:text-3xl text-2xl mb-4">
          {item.tagline}
        </h3>
      )}
      <div className="flex flex-col">
        <h1 className="lg:text-2xl text-lg font-bold text-PRIMARY-TEXT">
          Overview:
        </h1>
        <p className="text-gray-200 lg:text-md text-sm">{item.overview}</p>
      </div>
      <div className="flex flex-row flex-wrap gap-4 my-6">
        {item.genres &&
          item.genres.map((genre) => (
            <div
              key={genre.id}
              className="bg-RED-TEXT font-bold text-white px-2 rounded-md text-sm"
            >
              {genre.name}
            </div>
          ))}
      ...
      <div className="flex flex-row items-center space-x-6">
        ...
        {item.runtime && (
          <div className="flex flex-row items-center space-x-2">
            <h1 className="text-PRIMARY-TEXT font-semibold text-sm ">
              Runtime:
            </h1>
      ...
      {item.created_by && item.created_by.length > 0 && (
        <div className="flex my-4 flex-row items-center space-x-4">
          <h1 className="text-xl font-bold text-PRIMARY-TEXT">Creator:</h1>
